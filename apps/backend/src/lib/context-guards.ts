...
export class ContextGuard {
   ...
   */
  static isModelSuitableForStep(modelName: string, stepId: string, phase?: string): {
    suitable: boolean;
    reason?: string;
    ...
    if (model.maxTokens <= 8192 && LARGE_CONTEXT_STEPS.includes(stepId)) {
      return {
        ...
        suggestedModel: HIGH_QUALITY_STEPS.includes(stepId) ? "gpt-4" : "gpt-4o-mini"
      };
    }
  ...
  static enforceModelSelection(
    ...
    console.log(`🔄 Overriding ${originalModel} → ${finalModel} for ${stepId}${phase ? ` (${phase})` : ""}`);
    ...
    if (jobId) {
      traceSystem.addEntry(jobId, {
        ...
        sources: [`Context guard overrode ${originalModel} due to: ${check.reason}`],
        hash: ""
      });
    }
  ...
  static preflightCheck(
    ...
    for (const [step, model] of Object.entries(modelMapping)) {
      const enforcement = this.enforceModelSelection(model, step, undefined, jobId);
    ...
    if (!passed) {
      console.warn(`⚠️  Preflight check found ${violations.length} model violations:`);
   ...
   */
  static requiresPhaseSplitting(stepId: string, modelName: string, estimatedTokens: number): boolean {
    const model = MODEL_CAPABILITIES[modelName];
  ...
  static estimateCost(modelName: string, inputTokens: number, outputTokens: number): number {
    ...
    return (inputTokens / 1000 * model.inputCost) + (outputTokens / 1000 * model.outputCost);
  }
}

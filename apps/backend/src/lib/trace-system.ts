...
class TraceSystem {
  ...
  addEntry(jobId: string, entry: Partial<TraceEntry>): void {
    ...
    const fullEntry: TraceEntry = {
      ...
      prompt_tokens_est:
        entry.prompt_tokens_est || Math.ceil((entry.prompt_bytes || 0) / 4),
      truncate_applied: entry.truncate_applied || false,
      sources_after_filter:
        entry.sources_after_filter || entry.source_count_after_filter || 0,
      ...
      input_tokens_est:
        ...
        entry.prompt_tokens_est ||
        Math.ceil((entry.prompt_bytes || 0) / 4),
      max_tokens: entry.max_tokens || 1200,
      source_count_after_filter:
        entry.source_count_after_filter || entry.sources_after_filter || 0,
      ...
      token_efficiency:
        entry.actual_tokens &&
        (entry.input_tokens_est || entry.prompt_tokens_est)
    ...
    console.log(
      `🔍 [TRACE] Added entry for ${jobId}: ${fullEntry.step}${fullEntry.phase ? `(${fullEntry.phase})` : ""} - ${fullEntry.status}`,
    );
  }
  ...
  completeTrace(jobId: string, status: "completed" | "failed"): void {
    ...
    trace.total_duration_ms =
      new Date().getTime() - new Date(trace.created_at).getTime();

    console.log(
      `🔍 [TRACE] Completed trace for ${jobId}: ${status} (${trace.total_duration_ms}ms)`,
    );
  }
  ...
  cleanup(): void {
    ...
    if (traces.length > 100) {
      ...
      console.log(
        `🔍 [TRACE] Cleaned up old traces, keeping ${this.traces.size} traces`,
      );
    }
  }
  ...
  private updateSummary(trace: JobTrace, entry: TraceEntry): void {
    ...
    trace.summary.total_tokens_estimated = trace.entries.reduce(
      (sum, e) => sum + e.input_tokens_est,
      0,
    );
    trace.summary.total_tokens_actual =
      trace.entries.reduce((sum, e) => sum + (e.actual_tokens || 0), 0) ||
      undefined;
    trace.summary.total_rategate_wait_ms = trace.entries.reduce(
      (sum, e) => sum + e.rategate_wait_ms,
      0,
    );
    trace.summary.errors = trace.entries.filter(
      (e) => e.status === "error",
    ).length;

    // Update models_used count
    trace.summary.models_used[entry.model] =
      (trace.summary.models_used[entry.model] || 0) + 1;
  }

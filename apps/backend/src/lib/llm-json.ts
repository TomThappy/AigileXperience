 ...
 */
export async function llmJson<T>(options: LLMJsonOptions): Promise<LLMJsonResult<T>> {
  const {
    model,
    system,
    user,
  ...
  try {
    ...
    for (attempts = 1; attempts <= retries + 1; attempts++) {
      try {
        console.log(`🔄 [LLM-JSON] ${stepId}${phase ? `(${phase})` : ''} attempt ${attempts}/${retries + 1} with ${model}`);
        ...
        try {
          ...
          console.log(`✅ [LLM-JSON] ${stepId}${phase ? `(${phase})` : ''} successful JSON parse on attempt ${attempts}`);
          ...
          console.warn(`⚠️ [LLM-JSON] JSON parse failed on attempt ${attempts}: ${parseError}`);

          // 3) Try jsonrepair
          try {
            const repaired = jsonrepair(rawResponse);
            jsonData = JSON.parse(repaired);
            console.log(`🔧 [LLM-JSON] Successfully repaired JSON on attempt ${attempts}`);
            ...
            console.warn(`⚠️ [LLM-JSON] JSON repair also failed: ${repairError}`);

            // 4) If not the last attempt, try fixer-pass
            if (attempts <= retries) {
              console.log(`🔄 [LLM-JSON] Trying fixer-pass for attempt ${attempts + 1}`);
              
              rawResponse = await callLLMWithStructuredOutput({
                model,
                system: 'You are a JSON fixer. Output ONLY valid minified JSON matching the schema. No prose, no explanations, no markdown.',
            ...
            lastError = new Error(`JSON parsing failed after ${attempts} attempts: ${parseError}`);
          }
        }

      } catch (llmError) {
        console.error(`❌ [LLM-JSON] LLM call failed on attempt ${attempts}:`, llmError);
    ...
    console.error(`❌ [LLM-JSON] Final failure after ${attempts - 1} attempts: ${errorMessage}`);
    ...
    throw new Error(`LLM JSON failed after ${attempts - 1} attempts: ${errorMessage}`);

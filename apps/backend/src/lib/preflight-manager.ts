...
export class PreflightManager {
  ...
  async performPreflight(
    ...
    console.log(`🔍 [PREFLIGHT] ${stepId} (${model}): ${totalTokens}/${availableTokens} tokens`);

    // Apply truncation if needed
    if (totalTokens > availableTokens) {
      console.log(`⚠️ [PREFLIGHT] Token limit exceeded for ${stepId}, applying truncation...`);
    ...
    if (jobId && truncateApplied) {
      console.log(`📊 [PREFLIGHT] Applied truncation: sources ${sources.length}→${adjustedSources.length}, brief ${brief.length}→${adjustedBrief.length} chars`);
    }
  ...
  private truncateSources(stepId: string, sources: any[]): any[] {
    const stepFilters: Record<string, { maxSources: number; maxCharsPerSource: number; keywords?: string[] }> = {
      evidence: { maxSources: 20, maxCharsPerSource: 1000 },
      brief: { maxSources: 8, maxCharsPerSource: 800, keywords: ["evidence", "analysis"] },
      problem: { maxSources: 6, maxCharsPerSource: 600, keywords: ["problem", "issue", "challenge", "pain"] },
      solution: { maxSources: 6, maxCharsPerSource: 600, keywords: ["solution", "product", "technology", "approach"] },
      team: { maxSources: 4, maxCharsPerSource: 600, keywords: ["team", "founder", "experience", "background"] },
      market: { maxSources: 8, maxCharsPerSource: 800, keywords: ["market", "industry", "sector", "size", "growth"] },
      business_model: { maxSources: 6, maxCharsPerSource: 600, keywords: ["revenue", "pricing", "model", "monetization"] },
      competition: { maxSources: 6, maxCharsPerSource: 600, keywords: ["competitor", "alternative", "comparison"] },
      status_quo: { maxSources: 4, maxCharsPerSource: 600, keywords: ["current", "existing", "traditional"] },
      gtm: { maxSources: 6, maxCharsPerSource: 600, keywords: ["marketing", "sales", "customer", "channel"] },
      financial_plan: { maxSources: 8, maxCharsPerSource: 800, keywords: ["financial", "funding", "revenue", "cost"] },
      investor_score: { maxSources: 8, maxCharsPerSource: 600 },
    };

    const filter = stepFilters[stepId] || { maxSources: 6, maxCharsPerSource: 600 };
    let filteredSources = sources.slice(0, filter.maxSources);

    // Apply keyword filtering if specified
    if (filter.keywords) {
      filteredSources = this.filterSourcesByKeywords(sources, filter.keywords, filter.maxSources);
    }
        ...
        ['content', 'text', 'description', 'summary'].forEach(field => {
          if (truncated[field] && typeof truncated[field] === 'string') {
            if (truncated[field].length > filter.maxCharsPerSource) {
              truncated[field] = truncated[field].substring(0, filter.maxCharsPerSource) + '...';
            }
          }
        });
   ...
   */
  private filterSourcesByKeywords(sources: any[], keywords: string[], maxSources: number): any[] {
   ...
   */
  shouldForcePhaseSplitting(stepId: string, estimatedTokens: number, modelCapabilities: any): boolean {

...
export default async function autoV1(app: FastifyInstance) {
  app.post("/api/auto/run", async (req, reply) => {
    ...
    try {
      ...
      app.log.info(`[${traceId}] Request body:`, { 
        project_title: b.project_title?.substring(0, 50) + '...', 
        elevator_pitch: b.elevator_pitch?.substring(0, 50) + '...',
        hasTitle: !!b.project_title,
        hasPitch: !!b.elevator_pitch
      });
      ...
      app.log.info(`[${traceId}] Pipeline completed successfully`, {
        duration: Date.now() - startTime,
        responseSize: JSON.stringify(final).length,
      ...
      app.log.error(`[${traceId}] Pipeline failed after ${duration}ms:`, {
